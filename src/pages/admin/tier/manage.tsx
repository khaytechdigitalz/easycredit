...
// ----------------------------------------------------------------------
// Define the specific structure of the data row (account tier)
...
const TABLE_HEAD: HeadLabel[] = [
  ...
  { id: 'benefits', label: 'benefits', align: 'left' },
  { id: 'description', label: 'description', align: 'left' },
  { id: 'tier', label: 'tier', align: 'left' },
  { id: 'withdrawal', label: 'withdrawal', align: 'left' },
  { id: 'deposit', label: 'deposit', align: 'left' },
  { id: 'actions', label: 'Action', align: 'center' }, // Corrected ID to 'actions'
];

// --- Form Definitions for account tier ---

// 1. Schema for account tier (Corrected from account tier schema)
const AccountTierSchema = Yup.object().shape({
...
interface AccountTierFormValuesProps {
  benefits: string;
  description: string;
  withdrawal: string;
  deposit: string;
...
// ----------------------------------------------------------------------
const AccountTierEditFormModal = ({ onClose, onSubmitted, currentData }: { onClose: () => void; onSubmitted: () => void; currentData: ICustomerItem | null }) => {
  ...
  const defaultValues = useMemo(() => ({
    ...
    withdrawal: currentData?.withdrawal || '',
    deposit: currentData?.deposit || '',
  }), [currentData]);
 
  // Form methods are defined LOCALLY within the component
  const methods = useForm<AccountTierFormValuesProps>({
    resolver: yupResolver(AccountTierSchema),
    defaultValues,
  });
  ...
  // Corrected onSubmit to patch to the account tier endpoint and handle success/error
  const onSubmit = async (formState: AccountTierFormValuesProps) => {
    ...
    try {
      ...
      const response = await axios.patch(`/admin/account-tier/${currentData._id}`, {
      ...
      enqueueSnackbar(response.data.message || 'Tier data updated successfully!');
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <Card sx={{ p: 3 }}>
              ...
              >
                <RHFTextField name="benefits" label="Tier Benefit" />
                <RHFTextField name="description" label="Tier description" />
                <RHFTextField name="withdrawal" label="Tier withdrawal" />
                <RHFTextField name="deposit" label="Tier deposit" />
                <RHFTextField name="tier" label="Tier Name" />
              </Box>
...
// 3. The account tier Form Component inside the Modal (Refactored and fixed)
const AccountTierFormModal = ({ onClose, onSubmitted }: { onClose: () => void; onSubmitted: () => void }) => {
  const { enqueueSnackbar } = useSnackbar();
  ...
  // Corrected onSubmit to post to the account tier endpoint and handle success/error
  const onSubmit = async (formState: AccountTierFormValuesProps) => {
    ...
    try {
      ...
      const response = await axios.post('/admin/account-tier', {
        benefits: formState.benefits,
        description: formState.description,
        deposit: formState.deposit,
        withdrawal: formState.withdrawal,
      ...
      console.log(response.data);
      enqueueSnackbar(response.data.message || 'New account tier data created successfully!');
          ...
          <Grid item xs={12} md={12}>
            <Card sx={{ p: 3 }}>
              ...
              >
                ...
                <RHFTextField name="withdrawal" label="Tier withdrawal" />
                <RHFTextField name="deposit" label="Tier deposit" />
                <RHFTextField name="tier" label="Tier tier" />
              </Box>
...
interface AccountTierTableRowProps {
  row: ICustomerItem;
  onEditRow: (row: ICustomerItem) => void;
}

function ContactTableRow({ row, onEditRow }: AccountTierTableRowProps) {
  const { _id, benefits, description,tier,withdrawal,deposit } = row;

  return (
    <TableRow hover>
      <TableCell sx={{ whiteSpace: 'nowrap' }}>{_id}</TableCell>
      <TableCell sx={{ whiteSpace: 'nowrap' }}>{benefits}</TableCell>
      <TableCell sx={{ whiteSpace: 'nowrap' }}>{description}</TableCell>
      <TableCell sx={{ whiteSpace: 'nowrap' }}>{tier}</TableCell>
      <TableCell sx={{ whiteSpace: 'nowrap' }}>{withdrawal}</TableCell>
      <TableCell sx={{ whiteSpace: 'nowrap' }}>{deposit}</TableCell>
...
export default function CustomerListPage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const requestResponse = await axios.get('/admin/account-tier', config);
        ...
        console.error("Error fetching account tier log:", error);
        enqueueSnackbar('Failed to fetch account tier log.', { variant: 'error' });
        setDashlog([]);
      } finally {
  ...
  const exportToCsv = () => {
    ...
    link.setAttribute('download', 'tier_data_list.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    enqueueSnackbar('Tier exported successfully!', { variant: 'success' });
  };
  ...
  return (
    <>
      <Head>
        <title> Account: Account Tier | Easy Credit</title>
      </Head>

      <Container maxWidth={themeStretch ? false : 'xl'}>
        <CustomBreadcrumbs
          heading="Manage Account Tier Data"
          links={[
      ...
      >
        ...
        <DialogContent dividers>
          <AccountTierFormModal onClose={handleCloseCreateModal} onSubmitted={handleFormSubmissionSuccess} />
        </DialogContent>
      </Dialog>
...
function applyFilter({
  ...
  if (filterName) {
    filteredData = filteredData.filter(
      (product) => product.benefits.toLowerCase().includes(filterName.toLowerCase())
    );
  }
